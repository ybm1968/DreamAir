...
- 임성민(팀장)
  - 주요 담당 : 항공권 구매
  - 프로젝트 설계 및 DB 구축, 항공권 조회/예매/결제, 탑승객 정보 입력, 결제 API 연동
- 한현진(팀원)
  - 주요 담당 : 게시판
...
## 7-1. 개별 평가
- 임성민
  - 항공권 조회부터 결제까지 모든 과정을 하나의 연결된 과정으로 진행 했습니다. 그래서 하나의 페이지가 이동 될 때마다 이전에 입력한 데이터를 계속 다음 페이지로 넘겨주고 탑승객 정보 입력 페이지와 결제 페이지에서 DB에 저장했습니다. 값을 계속 다음 페이지로 가져오는 부분이 어려웠고 더 좋은 방법이 있을지도 모르겠다는 고민을 해봤습니다. 그리고 처음으로 API를 적용해 봤는데 적용하기 전에는 어려울까봐 걱정했지만 생각보다 괜찮았고 다양한 기술을 적용해 볼 수 있는 좋은 경험이었습니다.
- 한현진
